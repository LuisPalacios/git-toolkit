...
const v2JSON = `{
    ...
    "global": {
        ...
        "credential_gcm": { "enabled": true, "helper": "manager", "credential_store": "wincredman" }
    },
    "accounts": {
        "GitHub-Test": {
            "provider": "github",
            ...
            "gcm": { "provider": "github", "use_http_path": false }
        },
        "Forgejo-Homelab": {
            "provider": "forgejo",
...
const v1JSON = `{
    ...
    "accounts": {
        ...
        "Gitea-Generic": {
            "url": "https://git.example.org/luis",
            "username": "luis",
            "folder": "gitea-luis",
            "name": "Luis",
...
const v1RealWorldJSON = `{
    ...
    "accounts": {
        "git-parchis-familia": {
            "url": "https://git.parchis.org/familia",
            "username": "luis", "folder": "git-parchis-familia",
            "name": "Luis Palacios", "email": "luis@gmail.com",
            "gcm_provider": "generic",
            "repos": { "ines-denia": { "credential_type": "gcm" } }
        },
        "git-parchis-infra": {
            "url": "https://git.parchis.org/infra",
            "username": "luis", "folder": "git-parchis-infra",
            "name": "Luis Palacios", "email": "luis@gmail.com",
            "gcm_provider": "generic",
            "repos": {
                "homelab-ops": { "credential_type": "gcm" },
                "migra-forgejo": { "credential_type": "gcm" }
            }
        },
        "git-parchis-luis": {
            "url": "https://git.parchis.org/luis",
            "username": "luis", "folder": "git-parchis-luis",
            "name": "Luis Palacios", "email": "luis@gmail.com",
            "gcm_provider": "generic",
            "repos": {
        ...
        "github-LuisPalacios": {
            ...
            "username": "LuisPalacios", "folder": "github-luispa",
            "name": "Luis Palacios", "email": "luis@gmail.com",
            "gcm_provider": "github",
            "ssh_host": "gh-LuisPalacios", "ssh_hostname": "github.com", "ssh_type": "ed25519",
            "repos": {
        ...
        "github-readonly": {
            "url": "https://github.com",
            "username": "LuisPalacios", "folder": "github-readonly",
            "name": "Luis Palacios", "email": "luis@gmail.com",
            "gcm_provider": "github",
            "repos": { "kollective-networks/kltv.kombine": { "credential_type": "gcm" } }
        },
        "github-sumwall": {
            "url": "https://github.com/Sumwall",
            "username": "SW-Luis-Palacios", "folder": "github-sumwall",
            "name": "SW-Luis", "email": "luis@sumwall.com",
            "gcm_provider": "github",
            "repos": {
                "sumwall.browser": { "credential_type": "gcm" },
                "sumwall.docs": { "credential_type": "gcm" }
            }
        },
        "github-sumwall-rest": {
            "url": "https://github.com/Sumwall",
            "username": "SW-Luis-Palacios", "folder": "github-sumwall-rest",
            "name": "SW-Luis", "email": "luis@sumwall.com",
            "gcm_provider": "github",
            "repos": { "sumwall.github.io": { "credential_type": "gcm" } }
        }
    }
}`
...
func TestParseV1RealWorldDedup(t *testing.T) {
	...
	// git-parchis (3 merged), github-LuisPalacios (2 merged), github-sumwall (2 merged)
	...
	// --- git-parchis: 3 v1 accounts merged ---
	parchisAcct, ok := cfg.Accounts["git-parchis"]
	if !ok {
		// Try to find it by iterating.
		for k, a := range cfg.Accounts {
			if a.Username == "luis" && extractHostFromURL(a.URL) == "git.parchis.org" {
				t.Logf("parchis account found as %q", k)
				parchisAcct = a
				ok = true
				break
			}
	...
	if !ok {
		t.Fatal("parchis account not found")
	}
	if parchisAcct.URL != "https://git.parchis.org" {
		t.Errorf("parchis URL = %q", parchisAcct.URL)
	}

	// Find parchis source.
	var parchisSrc Source
	for k, s := range cfg.Sources {
		if s.Account == "git-parchis" || (cfg.Accounts[s.Account].Username == "luis" && extractHostFromURL(cfg.Accounts[s.Account].URL) == "git.parchis.org") {
			parchisSrc = s
			t.Logf("parchis source key: %q, repos: %d", k, len(s.Repos))
			break
		}
	}
	// Should have 4 repos: familia/ines-denia, infra/homelab-ops, infra/migra-forgejo, luis/config-json.
	if len(parchisSrc.Repos) != 4 {
		t.Errorf("parchis repos = %d, want 4", len(parchisSrc.Repos))
		for k := range parchisSrc.Repos {
			t.Logf("  repo: %q", k)
		}
	}
	// Check org/ prefixes.
	for _, expected := range []string{"familia/ines-denia", "infra/homelab-ops", "infra/migra-forgejo", "luis/config-json"} {
		if _, ok := parchisSrc.Repos[expected]; !ok {
			t.Errorf("missing repo %q", expected)
		}
	}
	// CloneFolder override preserved (was explicit folder in v1).
	if r, ok := parchisSrc.Repos["luis/config-json"]; ok {
		if r.CloneFolder != "~/.config/git-config-repos" {
			t.Errorf("config-json clone_folder = %q", r.CloneFolder)
		}
	}

	// --- github-LuisPalacios: 2 merged (personal + readonly) ---
	var ghSrc Source
	var ghAcctKey string
	for k, s := range cfg.Sources {
		acct := cfg.Accounts[s.Account]
		if acct.Username == "LuisPalacios" {
			ghSrc = s
			ghAcctKey = s.Account
			t.Logf("github-LuisPalacios source key: %q, account: %q, repos: %d", k, s.Account, len(s.Repos))
			break
		}
	}
	ghAcct := cfg.Accounts[ghAcctKey]
	if ghAcct.SSH == nil || ghAcct.SSH.Host != "gh-LuisPalacios" {
		t.Error("github SSH should be preserved from the account that had it")
	}
	// Should have: LuisPalacios/git-toolkit, LuisPalacios/dotfiles, kollective-networks/kltv.kombine.
	...
	if _, ok := ghSrc.Repos["kollective-networks/kltv.kombine"]; !ok {
		t.Error("missing kollective-networks/kltv.kombine")
	}

	// --- github-sumwall: 2 merged (split account) ---
	var swSrc Source
	for _, s := range cfg.Sources {
		acct := cfg.Accounts[s.Account]
		if acct.Username == "SW-Luis-Palacios" {
			swSrc = s
			break
		}
	}
	// Should have 3 repos: Sumwall/sumwall.browser, Sumwall/sumwall.docs, Sumwall/sumwall.github.io.
	if len(swSrc.Repos) != 3 {
		t.Errorf("sumwall repos = %d, want 3", len(swSrc.Repos))
		for k := range swSrc.Repos {
			t.Logf("  repo: %q", k)
		}
	}
	// The "rest" repo should NOT have a folder override — new layout uses nested dirs.
	if r, ok := swSrc.Repos["Sumwall/sumwall.github.io"]; ok {
		if r.CloneFolder != "" {
			t.Errorf("sumwall.github.io should not have clone_folder override, got %q", r.CloneFolder)
		}
	}
}
